...
export class Header {
  ...
  headerInteraction() {
    if (myp5.mouseY < this.actualH && myp5.mouseY > 0) {
      this.actualH = Math.ceil(
        myp5.lerp(Math.ceil(this.actualH), this.headMax, 0.15 * mpy)
      )
      for (let btn of this.headerBtns) {
        btn.openAnimation()
      }
  ...
  drawHeader() {
    ...
    myp5.text(this.pageTitle, 32 * mpx * 0.95, this.headMin * mpy * 0.7 * 0.95)
    myp5.fill(ikariGray)
    myp5.text(this.pageTitle, 32 * mpx, this.headMin * mpy * 0.7)
    drawLogo(200, 300, 80)
    this.drawButtons()
    myp5.pop()
  }
